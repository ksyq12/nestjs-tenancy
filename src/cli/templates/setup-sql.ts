...
export function generateSetupSql(options: SetupSqlOptions): string {
  ...
  const lines: string[] = [
    ...
    `-- IMPORTANT: This script assumes all non-shared models have a '${tenantIdField}' column.`,
  ...
  for (const model of models) {
    ...
    lines.push(
      `CREATE INDEX IF NOT EXISTS tenancy_${safeName}_${safeTenantField}_idx ON ${tableName} (${tenantIdField});`,
    );
    lines.push(
